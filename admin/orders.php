...
admin_head('Заказы');
admin_topbar('Заказы', 'Управление заказами');
?>

<!-- Filters -->
<div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:20px;align-items:center">
  <a href="/admin/orders.php" class="btn btn-sm <?= !$statusFilter && !$typeFilter ? 'btn-primary' : 'btn-outline' ?>">Все</a>
  <a href="/admin/orders.php?status=new"       class="btn btn-sm <?= $statusFilter==='new' ? 'btn-primary' : 'btn-outline' ?>">Новые</a>
  <a href="/admin/orders.php?status=preparing" class="btn btn-sm <?= $statusFilter==='preparing' ? 'btn-primary' : 'btn-outline' ?>">Готовятся</a>
  <a href="/admin/orders.php?status=ready"     class="btn btn-sm <?= $statusFilter==='ready' ? 'btn-primary' : 'btn-outline' ?>">Готовы</a>
  <span style="color:var(--border)">|</span>
  <a href="/admin/orders.php?type=pickup"   class="btn btn-sm <?= $typeFilter==='pickup' ? 'btn-primary' : 'btn-outline' ?>">🏠 Самовывоз</a>
  <a href="/admin/orders.php?type=delivery" class="btn btn-sm <?= $typeFilter==='delivery' ? 'btn-primary' : 'btn-outline' ?>">🚴 Доставка</a>
</div>

<div class="table-card">
  <div class="table-card-header">
    <h3>Заказы <span style="color:var(--text-muted);font-size:0.85rem">(<?= count($orders) ?>)</span></h3>
